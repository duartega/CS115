...
def main():
    ...
    if decimals > 4 or decimals < 1:
        ...
        for i in range(num_locations):
            ...
            if wind_velocity < 0:
                print('Nice try! Even during time travel wind velo'
                      'city is positive, just going the other way!')
                exit(255)
            if wind_velocity > 767:
                print('You must be superman! Nice try stud..')
                exit(255)
        ...
        print('\tLocation with lowest air temperature: ',lowestAirTempPlace,' (',round(lowestAirTemp,decimals),' F)',sep="")
        print('Wind Velocities\n\tAvg recorded wind velocity: ',round(averageWindVelocity,decimals),'mph')
        print('\tLocation with highest wind velocity: ', highestWindVelocityPlace,' (',round(highestWindVelocity,decimals),' mph)',sep='')
